...
- Backend must be up (`docker compose up -d`); gateway URL is `AppConfig.GatewayBaseUrl` (localhost:8090 on Windows; Android will need the PC's LAN IP + cleartext-HTTP config).
