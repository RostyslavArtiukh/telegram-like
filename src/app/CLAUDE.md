...
## Build & run (Windows)
- **NOT in `TelegramLike.sln`** — CI builds that on ubuntu and a Windows-TFM MAUI project would break it. Use `TelegramLike.App.slnx` (app + Client) or build the csproj directly. Contracts left the slnx in [TL-120]: it now arrives through the SDK as a package, so **run `./build/pack-shared.ps1` once before building the app**.
...
- **Full-height chat layout:** the chat screen is a flex column (`.chat-shell` in `wwwroot/app.css`, height `calc(100dvh - var(--mud-appbar-height) - 32px)`) — header + composer stay put, only the message list (`.chat-messages`, `flex:1;overflow-y:auto`) scrolls, and it auto-pins to the newest message via `telegramLike.scrollToBottom` (JS in `index.html`) whenever `shouldScrollToBottom` is set (initial load, message sent/received — *not* when paging older history). Works the same on the desktop window and the phone ([TL-108]).
