...
public interface IChatMembershipReadModel
{
    ...
    Task<bool> IsModeratorAsync(Guid chatId, Guid userId, CancellationToken ct = default);
    ...
    Task UpsertActiveAsync(Guid chatId, Guid userId, string? role, DateTime occurredAt, CancellationToken ct = default);

    Task DeactivateAsync(Guid chatId, Guid userId, DateTime occurredAt, CancellationToken ct = default);

    Task SetRoleAsync(Guid chatId, Guid userId, string role, DateTime occurredAt, CancellationToken ct = default);
}
