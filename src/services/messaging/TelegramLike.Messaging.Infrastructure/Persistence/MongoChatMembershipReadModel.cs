...
internal sealed class MongoChatMembershipReadModel(IMongoDatabase database) : IChatMembershipReadModel
{
    ...
    // Last-writer-wins by occurredAt via a conditional pipeline update: the new state is
    // applied only when occurredAt is newer than the stored LastEventAt (missing => epoch).
    // A stale event is a no-op, never a resurrect/delete. One atomic upsert, so
    // concurrent/redelivered events can't interleave into a wrong final state.
