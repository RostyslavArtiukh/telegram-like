namespace TelegramLike.Messaging.Domain;

/// <summary>
/// Base type for deliberate business-rule violations raised by the domain / application layer
/// (e.g. "message not found", "message text cannot exceed …", "you are not an active member").
/// The API layer maps these to <c>400</c> and surfaces the message. Framework-thrown exceptions
/// (LINQ, the Mongo driver) are intentionally <b>not</b> this type, so they bubble up as a
/// <c>500</c> instead of being mislabelled as a client <c>400</c> with an internal message
/// leaked in the body.
/// </summary>
