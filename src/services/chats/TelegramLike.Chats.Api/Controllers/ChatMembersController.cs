...
public sealed class ChatMembersController : ApiControllerBase
{
    ...
    public async Task<IActionResult> Join(Guid chatId, CancellationToken ct)
    {
        if (!TryGetUserId(out var userId)) return Unauthorized();
        await _mediator.Send(new JoinChatCommand(chatId, userId), ct);
        return NoContent();
    }

    [HttpPost("{chatId:guid}/leave")]
    public async Task<IActionResult> Leave(Guid chatId, CancellationToken ct)
    {
        if (!TryGetUserId(out var userId)) return Unauthorized();
        await _mediator.Send(new LeaveChatCommand(chatId, userId), ct);
        return NoContent();
    }

    [HttpPost("{chatId:guid}/members/{targetUserId:guid}/kick")]
    public async Task<IActionResult> KickMember(Guid chatId, Guid targetUserId, CancellationToken ct)
    {
        if (!TryGetUserId(out var actorId)) return Unauthorized();
        await _mediator.Send(new KickMemberCommand(chatId, targetUserId, actorId), ct);
        return NoContent();
    }

    [HttpPost("{chatId:guid}/members/{targetUserId:guid}/role")]
    public async Task<IActionResult> ChangeMemberRole(
        Guid chatId, Guid targetUserId, [FromBody] ChangeMemberRoleRequest body, CancellationToken ct)
    {
        if (!TryGetUserId(out var actorId)) return Unauthorized();
        await _mediator.Send(new ChangeMemberRoleCommand(chatId, targetUserId, body.NewRole, actorId), ct);
        return NoContent();
    }

    [HttpPost("{chatId:guid}/transfer-ownership")]
    public async Task<IActionResult> TransferOwnership(
        Guid chatId, [FromBody] TransferOwnershipRequest body, CancellationToken ct)
    {
        if (!TryGetUserId(out var actorId)) return Unauthorized();
        await _mediator.Send(new TransferOwnershipCommand(chatId, body.NewOwnerUserId, actorId), ct);
        return NoContent();
    }
}
