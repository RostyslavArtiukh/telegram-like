...
/// <summary>
/// Request binding: BFF-enriched fields (recipients, isBroadcast, actorIsPremium,
/// actorIsModerator) bind from JSON body into the exact command properties.
    ...
    : IClassFixture<MessagingApiFactory>
{
    ...
    // ── AddReaction: Emoji binds as enum name string, actorIsPremium binds ─

    [Fact]
    public async Task AddReaction_BindsEmojiAndActorIsPremium()
    {
        factory.Mediator
            .Send(Arg.Any<IRequest<Unit>>(), Arg.Any<CancellationToken>())
            .Returns(Unit.Value);

        // The BFF sends the emoji name ("Heart") and actorIsPremium flag.
        var body = Json(new { emoji = "Heart", actorIsPremium = true });
    ...
    // ── RetractMessage: actorIsModerator (BFF-enriched) binds ─────────────

    [Fact]
    public async Task Retract_BindsActorIsModerator()
    {
        factory.Mediator
            .Send(Arg.Any<IRequest<Unit>>(), Arg.Any<CancellationToken>())
            .Returns(Unit.Value);

        var body = Json(new { actorIsModerator = true });
    ...
    public async Task HideMessage_BindsMessageIdAndActor()
    {
        ...
        await factory.Mediator.Received(1).Send(
            Arg.Is<HideMessageCommand>(c =>
                c.MessageId == MessageId && c.UserId == ActorId),
            Arg.Any<CancellationToken>());
    }
}
