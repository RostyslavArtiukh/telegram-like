...
- **Shared projects** — `src/shared/`, one per layer so a layer only drags its own dependencies:
  ...
  - `TelegramLike.Shared.Application`: `ValidateRequestBeforeHandling` (MediatR pipeline validation), `IIntegrationEventMapper` (change event → integration event, keyed by `ChangeEventType`).
