...
- **Outbox**: `outgoing_events` Mongo-колекція + `OutgoingEventsStore` (internal). Поля: `Id`, `EventType` (**оголошене wire-ім'я** `context.event.v1` з атрибута `[IntegrationEventName]`, з [TL-117]; до того — CLR-ім'я типу, що ламало рядки при перейменуванні record-а), `Payload` (JSON), `OccurredAt`, `SentAt?`, `Retries`. З [TL-75] також claim/lease 60с — без дублювання publish під >1 репліку.
- **OutgoingEventsSender** — BackgroundService, кожні `OutgoingEvents:PollIntervalSeconds` сек тягне `SentAt == null && DeadLetteredAt == null`, deserialize по `IntegrationEventNames.Resolve(EventType)` (реєстр wire-імен + fallback на `Type.GetType` для legacy-рядків), `IPublishEndpoint.Publish(payload, type)`, `MarkSentAsync`. На exception — `RecordFailureAsync(id, error, maxRetries)` (інкрементить Retries + пише LastError + ставить DeadLetteredAt коли досягло MaxRetries, Step 23).
...
2. Створити integration event у `src/TelegramLike.Contracts/<Context>/XIntegrationEvent.cs` **з `[IntegrationEventName("context.event.v1")]`** — без нього падає `IntegrationEventNamesTests` (і writer кинув би на першій публікації). Ім'я не міняти НІКОЛИ після того, як рядки його понесли: нова форма = новий тип із `.v2`.
