...
**Android ЕМУЛЯТОР ([TL-106], 2026-07-20) — ЗРОБЛЕНО й live-verified В ЕМУЛЯТОРІ (без фізичного телефона).** Ціль юзера: ганяти mobile-версію на ПК через емулятор мобільного пристрою, максимально схоже на веб/десктоп.
- **Мережа:** `AppConfig.GatewayBaseUrl` під `#if ANDROID` тепер `http://10.0.2.2:18090` — магічний аліас емулятора на loopback ХОСТА (НЕ LAN IP, НЕ firewall; LAN-варіант `192.168.0.101` лишив у коментарі для фізичного пристрою). cleartext уже дозволений у маніфесті.
- **Оточення на машині:** `maui-android` workload + Android SDK (`%LOCALAPPDATA%\Android\Sdk`) + adb вже стояли; `JAVA_HOME` був БИТИЙ (вказував на видалений `jdk-20`) → виправив на `C:\Program Files\Java\jdk-17` (User env, збережено). Emulator 36.6.11 + **WHPX** (`emulator -accel-check` OK; VT-x увімкнено). Docker Desktop піднімає стек паралельно — не конфліктує.
- **AVD:** `telegramlike` (Android 15 / API 35, `google_apis;x86_64`, профіль pixel_7).
- **ГРАБЛІ system-image (головна причина затримки):** `sdkmanager "system-images;android-35;google_apis;x86_64"` на повільному каналі (~1.5 МБ/с) рестартить ~1.6 ГБ з нуля при кожному обриві й падає ("Warning: Failed to download package!"). Обхід що спрацював: резюмований `curl -L -C - --retry 999 --retry-all-errors -o img.zip https://dl.google.com/android/repository/sys-img/google_apis/x86_64-35_r09.zip` → `unzip` у `system-images/android-35/google_apis/` (у zip тека `x86_64/` з `source.properties` — avdmanager підхоплює образ БЕЗ реєстрації sdkmanager'ом, `package.xml` не обов'язковий). Пакет `emulator` (972 МБ) sdkmanager докачав сам.
- **Деплой+запуск:** `dotnet build src/app/TelegramLike.App/TelegramLike.App.csproj -f net10.0-android -t:Run` (build+install+launch на завантажений емулятор). Драйв headless: `adb shell input tap/text` + `adb exec-out screencap -p > shot.png`.
- **Verified в емуляторі (весь ланцюг, ідентичний web/desktop):** register (identity через gateway) → sign in (обмін на JWT) → Chats (порожній) → create group "EmuChat" (presence "1 online") → send "Hello-from-the-emulator" (бульбашка з реакціями/діями). Той самий MudBlazor UI.
- **iOS НЕможливий на Windows** — навіть iOS-симулятор потребує macOS+Xcode; на цій машині нема способу. Тому mobile-перевірка = Android-емулятор.
- Емулятор-воркфлоу продубльований у `src/app/CLAUDE.md`.
