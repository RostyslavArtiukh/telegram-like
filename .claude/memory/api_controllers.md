...
- `Controllers/ApiControllerBase.cs` — `[ApiController]` base; `TryGetUserId(out)` / `CurrentUserId` resolve the actor from JWT `sub` (fallback `ClaimTypes.NameIdentifier`); relies on `MapInboundClaims=false`. Copied verbatim per service (namespace only) — matches the autonomous-bundle convention, no shared project.
