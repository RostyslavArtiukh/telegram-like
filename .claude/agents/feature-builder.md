...
- **Service layering:** Domain (aggregate/VO/events) → Application (MediatR command/query + FluentValidation validator) → Infrastructure (Mongo repo; outbox mapper if it raises a domain event) → Api (`[ApiController]` controllers in `Controllers/`, split by responsibility — group by the resource mutated; thin → delegate to MediatR; `ApiControllerBase` reads the actor from JWT `sub`; a global `DomainExceptionFilter` maps domain exceptions → status/body — **preserve the service's existing wire contract**, never a blanket copy; request/response records in `Contracts/`; wire with `AddControllers` + `MapControllers`, keep the `JsonStringEnumConverter` only where the service already had it, health endpoints stay minimal). See the `api_controllers` memory.
