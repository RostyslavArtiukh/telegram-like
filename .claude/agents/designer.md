...
- **MAUI app** — everything through the SDK: `TelegramLikeSession` auth, typed clients, `ITelegramLikeRealtimeClient` for real-time (`JoinChatAsync` per open chat; hub events fire on background threads → always `InvokeAsync`). Pushes are id-only signals — refetch the entity over HTTP.
