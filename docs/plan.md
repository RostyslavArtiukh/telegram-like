...
> ⚠️ **ІСТОРИЧНИЙ ДОКУМЕНТ (2026-05).** Це початковий 8-денний план для *монолітної* версії (місцями згадує PostgreSQL / нерозв'язаний стек). Проєкт відтоді мігрував у мікросервіси + Blazor BFF. Актуальна архітектура — у кореневому `CLAUDE.md` та директорних `CLAUDE.md`. Лишено для історії.
