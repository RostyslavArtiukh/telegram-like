...
> ℹ️ **Датовано 2026-05, до розділення на мікросервіси.** Доменна модель/ubiquitous language переважно ще актуальні, але «bounded contexts» тут — попередній поділ; фактичні межі сервісів дивись у кореневому + директорних `CLAUDE.md`.
