...
> ℹ️ **Частково застаріло (2026-05).** Описує *одну спільну* базу для всіх контекстів; тепер кожен мікросервіс має власну БД (`telegramlike_<service>`). Колекція `user_chat_settings` тут описана, але не реалізована. Опис колекцій/полів переважно ще коректний як доменна довідка; топологію дивись у директорних `CLAUDE.md`.
